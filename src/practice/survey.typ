...
// Числовая оценка 1–5: рамка с вписанным значением.
#let _rating(value) = box(
  stroke: (bottom: 0.5pt),
  width: 1cm,
  inset: (bottom: 2pt),
  align(center, if value != none { [#value] } else { sym.zws }),
)
...
#let practice-report-survey(
  ...
  let q-block(num, text-body, body) = block(
    width: 100%, breakable: false, spacing: 0.6em,
  )[
    #num #text-body
    #body
  ]

  q-block([1], [Удовлетворены ли Вы местом прохождения практики? (один ответ)],
    _single-choice(_q1-options, answers.at("q1", default: none)),
  )

  q-block([2], [Удовлетворены ли Вы качеством разработки рабочей программы практики (РПП)? (один ответ)],
    _single-choice(_q2-options, answers.at("q2", default: none)),
  )

  q-block([3], [Считаете ли Вы достаточными для выполнения работ, предусмотренных РПП, теоретические знания, которые Вы получили в Университете? (один ответ)],
    _single-choice(_q3-options, answers.at("q3", default: none)),
  )

  q-block([4], [Дала ли производственная практика возможность применить и развить навыки, необходимые для дальнейшей профессиональной деятельности (в соответствии с Вашим направлением подготовки/Вашей специальностью)? (один ответ)],
    _single-choice(_q4-options, answers.at("q4", default: none)),
  )

  q-block([5], [Хотели бы Вы в дальнейшем продолжить свою трудовую деятельность в организации, в которой проходили практику? (один ответ)],
    _single-choice(_q5-options, answers.at("q5", default: none)),
  )

  q-block([6], [Как Вы оцениваете итоги практики с точки зрения её результативности? (один ответ)],
    _single-choice(_q6-options, answers.at("q6", default: none)),
  )

  q-block([7], [Оцените степень удовлетворённости местом прохождения практики (материально-техническая оснащённость, кадровый состав) по пятибалльной шкале (1 — очень плохо, 5 — отлично)],
    _rating(answers.at("q7", default: none)),
  )

  q-block([8], [Оцените степень удовлетворённости взаимоотношениями с руководителем практики от профильной организации по пятибалльной шкале (1 — очень плохо, 5 — отлично)],
    _rating(answers.at("q8", default: none)),
  )

  q-block([9], [Оцените степень удовлетворённости взаимоотношениями с руководителем практики от Университета по пятибалльной шкале (1 — очень плохо, 5 — отлично)],
    _rating(answers.at("q9", default: none)),
  )

  q-block([10], [Оцените удовлетворённость в целом условиями прохождения практики (один ответ)],
    _single-choice(_q10-options, answers.at("q10", default: none)),
  )

  // Вопрос 11 — свободный текст в виде линий для рукописного заполнения.
  block(width: 100%, breakable: false, spacing: 0.6em)[
    ...
    #context {
      ...
      // Если есть ответ — выкладываем его как «значение над линиями»,
      // дополняя пустыми линиями до 3 шт.
